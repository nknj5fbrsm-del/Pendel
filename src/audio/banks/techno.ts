...
class TechnoPendulumVoice implements PendulumVoice {
  ...
  constructor(outputs: MixerOutputs) {
    ...
    const sawOpts = {
      ...
      filter: { Q: 1.8, type: "lowpass" as const, rolloff: -24 as const },
    ...
    const blueSquare = makeMonoVoice(
      blueDistort,
      { ...sawOpts, oscillator: { type: "square" } },
      blueNodes,
    );
    ...
    this.pink = new ScaleMelodyPlayer(
      WARM_MELODY_SCALE,
      [makeMonoVoice(pinkDistort, sawOpts, pinkNodes)],
      [0.08, 0.28],
      [0.3, 0.68],
    );
...
class TechnoBallVoice implements BallVoice {
  private readonly kick: any;
  private readonly snareNoise: any;
  private readonly snareBody: any;
...
export function createTechnoSoundBank(outputs: MixerOutputs): SoundBank {
  return {
    id: "techno",
