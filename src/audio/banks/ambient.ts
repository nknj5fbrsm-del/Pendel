...
class AmbientPendulumVoice implements PendulumVoice {
  private readonly blue: ScaleMelodyPlayer;
  private readonly pink: ScaleMelodyPlayer;
  private readonly clickPad: any;
  ...
  constructor(outputs: MixerOutputs) {
    ...
    this.blue = new ScaleMelodyPlayer(
      AMBIENT_BRIGHT_SCALE,
      [makeFmVoice(blueVerb, padOpts, 1, blueNodes)],
      [0.6, 2.2],
      [0.18, 0.42],
    );
...
class AmbientBallVoice implements BallVoice {
...
export function createAmbientSoundBank(outputs: MixerOutputs): SoundBank {
  return {
    id: "ambient",
    label: "Ambient Drone",
    pendulum: new AmbientPendulumVoice(outputs),
    ball: new AmbientBallVoice(outputs),
    applyMixerPresets(mixer: AudioMixer): void {
