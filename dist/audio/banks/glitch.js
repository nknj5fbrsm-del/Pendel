...
class GlitchPendulumVoice {
    constructor(outputs) {
        ...
        this.blue = new ScaleMelodyPlayer(GLITCH_SCALE, [
            makePluckVoice(outputs.melodyBlue, { attackNoise: 1.2, dampening: 8000, resonance: 0.98 }, 0.5, blueNodes, blueCheby),
            makePluckVoice(outputs.melodyBlue, { attackNoise: 0.6, dampening: 12000, resonance: 0.92 }, 0.5, blueNodes, blueCrush),
        ], [0.03, 0.12], [0.4, 0.85]);
        for (const n of blueNodes)
            this.blue.registerNode(n);
        this.pink = new ScaleMelodyPlayer(GLITCH_SCALE, [
            makePluckVoice(outputs.melodyPink, { attackNoise: 1, dampening: 6000, resonance: 0.96 }, 1, pinkNodes, pinkCheby),
        ], [0.04, 0.15], [0.35, 0.78]);
...
export function createGlitchSoundBank(outputs) {
    return {
        id: "glitch",
